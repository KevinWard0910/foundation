...
import com.kevin.base.utils.PageUtil;

/**
 * Created by kevin on 2018/11/29.
 * descript：例程 界面运行期间值会有变更时使用 observable 成员变量
...
public class ListViewModel <ItemViewModel extends XItemViewModel> extends ObservableArrayList<ItemViewModel> implements IListViewModel<ItemViewModel>{
    ...
    public void setHasMore(boolean hasMore) {
        mHasMore = hasMore;
//        if (mHasEndInfo) {
        ItemViewModel itemViewModel = getLastViewModel();
        if (itemViewModel != null) {
            itemViewModel.setLastItem(!hasMore);
//            }
        }
    }
    ...
    public boolean equals(Object o) {
        ...
        if (mViewModelValid.get() != that.mViewModelValid.get())
            return false;
